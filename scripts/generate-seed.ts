...
lines.push("insert into public.tests (slug, category_id, title, depth, emoji, is_premium, is_custom, questions) values");
...
for (const [title, questions] of Object.entries(PRESEEDED_TESTS)) {
  ...
  rows.push(
    `  (${sqlString(slug)}, ${category ? sqlString(category.id) : "null"}, ${sqlString(title)}, ${sqlString(depth)}, ${category ? sqlString(category.emoji) : "null"}, false, false, ${sqlJsonb(questions)})`,
  );
}
...
console.log(`Écrit : ${outPath.pathname} (${rows.length} tests pré-écrits, ${BADGES.length} badges)`);
